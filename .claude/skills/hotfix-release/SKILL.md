...
name: hotfix-release
description: |
  当用户要求"热修复/hotfix/紧急上线/线上 bug 修复"时触发。
  按 Hotfix 快速通道 6 步流程执行，跳过完整 SDLC 但保留质量门禁。
...
when-to-use: |
  线上紧急 bug 需要快速修复上线。
when-not-to-use: |
  - 非紧急变更（MUST 走完整 SDLC: requirement-analysis → coding → expert-review → ci-gate）
...
produces:
  - changes/proposals/<id>/retrospective.md（事后复盘）

requires:
  - skill: ci-gate
    condition: hotfix 快速 CI 通过
    error: CI 未通过，禁止上线
