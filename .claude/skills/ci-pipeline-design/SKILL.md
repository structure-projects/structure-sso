...
description: |
  当用户要求"写流水线/加 CI/CD/GitHub Actions/构建流水线"时触发。
  按生态三件套模板（build-and-push / release-maven / publish-npm）生成 GitHub Actions 流水线。
  MUST 手动触发（workflow_dispatch），禁止自动触发发布。
...
when-to-use: |
  为项目编写 CI/CD 流水线（GitHub Actions）。
when-not-to-use: |
  - 仅触发已有流水线（用 gh workflow run）
  - 项目已有完整流水线，仅修改
...
human-in-the-loop:
  - 流水线触发方式 MUST 用户确认
  - Secrets 配置 MUST 用户确认
  - 镜像仓库 / npm scope MUST 用户确认
