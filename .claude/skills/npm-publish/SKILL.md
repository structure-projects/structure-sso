...
name: npm-publish
description: |
  当用户要求"发布 npm 包/发 npm/npm publish"时触发。
  按生态规范发布 npm 包到 npmjs.com：@structure-projects scope + 校验 + 验证。
...
when-to-use: |
  发布 npm 包到 npmjs.com（仅 @structure-projects scope 的组件库）。
when-not-to-use: |
  - 发布业务包（业务包 MUST private: true）
  - 仅本地构建（用 npm run build）
...
require-confirm:
  - npm publish MUST 用户确认（生产操作）
  - 版本号 MUST 用户确认

on-failure: |
  发布失败 → 检查 npm token / 版本号冲突
  版本冲突 → 升版本号后重试
