...
name: docker-cli
description: |
  当用户要求"docker 命令/docker 操作/docker 构建/docker 运行/docker 日志"时触发。
  提供 docker CLI 常用命令的安全使用方式。
...
human-in-the-loop:
  - 生产环境的 docker 操作 MUST 用户确认
  - docker system prune 等清理操作 MUST 用户确认

on-failure: |
  命令失败 → 分析错误日志，修复后重试
  权限不足 → 引导用户配置 docker 权限
