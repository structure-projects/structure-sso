...
name: unit-testing
description: |
  当用户要求"写测试/补测试/跑测试/单测/write test/UT"时触发。
  MUST 为新代码编写单元测试并验证覆盖率。
  与 coding 技能并行执行。
...
when-to-use: |
  编码过程中或完成后需要编写/补充单元测试。
when-not-to-use: |
  - 仅做集成测试（用 integration-testing）
  - 仅做 E2E 测试（用 e2e-testing）
  - 无代码变更
...
human-in-the-loop:
  - 测试策略选择 MUST 与被测代码作者确认（如非同一人）

on-failure: |
  测试失败 → 分析是代码 bug 还是测试 bug；代码 bug 回到 coding
  覆盖率不达标 → 补充测试用例；不允许降低覆盖率阈值
