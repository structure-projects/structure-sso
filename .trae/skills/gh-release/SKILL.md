...
name: gh-release
description: |
  当用户要求"打 tag/发 Release/发布版本/GitHub Release"时触发。
  用 gh CLI 创建 GitHub Release，配合 changelog 生成 notes。
  这是 SDLC 的最后一步：archive-change 之后可选调用。
...
when-to-use: |
  变更已归档（archive-change 完成），需要打 tag 并创建 GitHub Release。
when-not-to-use: |
  - 变更未归档（MUST 先完成 archive-change）
  - 仅本地测试（无需 Release）
...
requires:
  - skill: archive-change
    condition: changes/archive/<id>/ 存在
    error: 变更未归档，MUST 先完成 archive-change

trust-level: standard

auto-decisions:
  - tag-naming          # v<X.Y.Z> 格式自动
  - release-title       # 与 tag 一致

require-confirm:
  - publish-release     # 创建 Release MUST 用户确认
  - trigger-pipeline    # 触发发布流水线 MUST 用户确认

on-failure: |
  tag 冲突 → 检查版本号是否已存在
  Release 失败 → 检查 changelog 是否存在
